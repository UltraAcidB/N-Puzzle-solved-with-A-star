...
class Puzzle:
    def __init__(self,puzzle,parent=None,move=None):
        ...
        self.parent=parent
        self.move=move
        self.blank=(0,0)#default position of blank
        self.g=0 #path cost
    ...
    def manhattanDistance(self):# taxicab geometry
        for i in range(len(self.state)):
            for j in range(len(self.state)):
                ...
                self.h+=distance#heuristics= sum of all the distance of all tiles
        return self.h
...
def aStar(problem,goal): 
    startNode=Puzzle(problem,None,None)#initiate the start
    endNode=Puzzle(goal,None,None)#initiate the goal
    ...
    while len(open_list):
        current=open_list[0]
        curr_index=0
        for index,item in enumerate(open_list):#find the node with the least value f(n) in the frontier
            if item.f<current.f:
                current=item
                curr_index=index
        ...
        children=current.expand()#generate children of current node
        ...
        for child in children:#loop through the children
            ...
            #this loop reduce the search space of the frontier
            for open_node in open_list:
                if child==open_node and child.g<open_node.g:
                    open_list.remove(open_node)
            #this loop reduce the potential of the same node appear in the explored
...
if __name__ == '__main__':
    ...
    #problem=[[0,6,2],[1,3,5],[7,4,8]] #medium: approximate 2min 
